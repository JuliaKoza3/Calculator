...
void check(Node<char>* node, Stack<char>& output, Stack<int> numbers)
{
    ...
    {
        int amountOfNumbers = node->next->data - '0';
        cout << 'M' << 'A';
        cout << 'X' << amountOfNumbers << ' ';
        numbers.printNumbers();
        cout << endl;
        //printSteps('X', numbers);
        ...
        if(amountOfNumbers!=1)
        {
            ...
            for (int i = 0; i < amountOfNumbers - 2; i++)
            {
                ...
                if (result > nextNumber || result == nextNumber)
                {
                    result = result;
                }
                else
